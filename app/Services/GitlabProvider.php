...
class GitlabProvider implements RepositoryProvider
{
    ...
    public function __construct(Client $httpClient, RepositoryFormatter $formatter)
    {
        ...
        $this->gitlabApiUrl = config('app.gitlab_api_url');
        $this->resultsPerPage = (int) config('app.results_per_page');
        $this->formatter = $formatter;
    }

    /**
     * Search for repositories on GitLab.
     *
    ...
    {
        $responseBody = $this->fetchDataFromGitlab($query);
     ...
     * @throws Exception If an error occurs.
     */
    private function fetchDataFromGitlab(string $query): string
    {
        try {
            $resp = $this->httpClient->get($this->gitlabApiUrl . '?per_page=' . $this->resultsPerPage . '&search='  . urlencode($query) . '&order_by=id&sort=asc');
    ...
    {
        return $this->formatter->format($repositories, 'gitlab');
    }
}
